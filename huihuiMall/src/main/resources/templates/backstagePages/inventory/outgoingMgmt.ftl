                                 ...
                                 <!--startprint-->
                                        <table class="table table-bordered" style="height:350px;border:2px solid black" width="100%">
                                            ...
                                            <tr>
                                                ...
                                                <td class="Consignee" colspan="4"></td>
                                            </tr>
                                            </tfoot>
                                            </table>
                                 <!--endprint-->
...
<script>
    ...
    function getOutgoingInfos(pageCount,currentPageNum){
        ...
        $.ajax({
            type : "GET", //用GET方式传输
            url : '${bath}/Inventory/queryOutInventory', //目标地址
            data : {
            ...
            success : function(retInfo) {
                ...
                if(retCode != 0){
                    ...
                    for(var i = 0 ;i < retDataLength; i ++){
                        ...
                        tbodyHtml += "<td>";
                        tbodyHtml += "<a href='#' data-toggle='modal' data-target='#detailModal' onclick='viewDetail(\""+orderNumber+"\");'>查看详情</a>";
                        tbodyHtml += "</td>";
                        tbodyHtml += "<td>";
                        tbodyHtml += "<button type='button' class='btn btn-default' data-toggle='modal' data-target='#detailModal2' style='margin:0px' onclick='Printing(\""+orderNumber+"\");'>打印</button>";
                        tbodyHtml += "</td>";
                        tbodyHtml += "<td>";
                        tbodyHtml += "<button type='button' class='btn btn-default' data-toggle='modal' data-target='#outStoreModal' style='margin:0px' onclick='openConfirmModal(\""+orderNumber+"\");'>出库</button>";
                        tbodyHtml += "</td>";
                        tbodyHtml += "</tr>";
    ...
    function viewDetail(orderNum){
        ...
        $.ajax({
            type : "GET", //用GET方式传输
            url : '${bath}/Inventory/outInventoryRecordDetail', //目标地址
            data : {
    ...
    function confirmOutgoing(ckOrderId){
        ...
        $.ajax({
            type : "POST", //用POST方式传输
            url : '${bath}/Inventory/OutInventory', //目标地址
            data : {
    ...
    function reset(){
     ...
     //浏览
    function Printing(orderNumber) {
        var orderNumParam = orderNumber;
        toastr.options = {
            "positionClass": "toast-tip-center-center", //弹出窗的位置
            "timeOut": "1000" //展现时间
        };
        $.ajax({
            ...
            success : function(retInfo) {
                ...
                if (retCode != 0){
                    ...
                    var retTotal = retData.orderTotal;//合计
                    var retCreateUser = retData.czName;//出库人
                    var deviceCode=retData.deviceCode;
                    var shopkeeperPhone=retData.shopkeeperPhone;
                    //缺  审批人  店主手机号
