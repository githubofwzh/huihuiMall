...
<script>
    ...
    $(document).ready(function() {
        ...
        //获取店主账号信息
        getShopKeeperInfos(pageCount,currentPageNum);

        //搜索点击
        $("#searchBtn").click(function(){
            $("#Tbody").empty();
            $("#loading").css("display","block");
            getShopKeeperInfos(pageCount,1);
        });
    ...
    //获取信息方法
    function getShopKeeperInfos(pageCount,currentPageNum){
        ...
        $.ajax({
            type : "POST", //用POST方式传输
            url : '${bath}/shopkeeper/queryAllShopKeeper', //目标地址
            data : {
                PageNum : currentPageNum,
                PageSize : pageCount,
                storeName : storeNameKeyParam,
                status : accountStatusSelectedParam
            },
    ...
    function openAndBan(accountId,accountStatus){
        ...
        $.ajax({
            type: "POST", //用POST方式传输
            url: '${bath}/shopkeeper/updateShopKeeper', //目标地址
            data: {
                id : accountIdParam,
                status : accountStatusParam
            },
            beforeSend: function (jqXHR) {
                if (undefined != header) {
                    jqXHR.setRequestHeader(header, token);
