...
<div class="container-fluid btn-operation-panel">
    <div class="row">
        <div class="col-lg-12 col-md-12 col-xs-12">
            ...
            <button type="button" class="btn btn-default" id="resetBtn" onclick="reset();"><i class="fa fa-undo fa-fw"></i> 重置</button>
        </div>
    </div>
</div>
...
<script>
    ...
    function getTransactionInfos(pageCount,currentPageNum){
        ...
        $.ajax({
            type : "POST", //用POST方式传输
            url : '${bath}/financial/queryAllPayInfo', //目标地址
            data : {
                PageNum : currentPageNum,
                PageSize : pageCount,
                outTradeNo : transactionCodeKeyParam,
                paytype : payTypeSelectedParam,
                paystatus : payStatusSelectedParam,
            ...
            success : function(retInfo) {
                ...
                if(retCode != 0){
                    ...
                    for(var i = 0; i < retDataLength; i++){
                        var index = i + 1;
                        var tradeNo = retData[i].trade_no?retData[i].trade_no:'';
                        var outTradeNum = retData[i].out_trade_no?retData[i].out_trade_no:'';
                        var payType = retData[i].payType;
                        var payStatus = retData[i].payStatus;
                        var money = retData[i].money;
                        var payTime = retData[i].payTime;
                        var phone = retData[i].phone?retData[i].phone:'';

                        var tbodyHtml = "<tr>";
                        tbodyHtml += "<td>"+index+"</td>";
                        tbodyHtml += " <td>"+tradeNo+"</td>";
                        ...
                        if(payStatus == 1){
                            tbodyHtml += "<td class='payMoney'>已支付</td>";
                        }else if(payStatus == 2){
                            tbodyHtml += "<td class='refundMoney'>已退款</td>";
                        }else{
                            tbodyHtml += "<td class='refundMoney'>未支付</td>";
                        }
