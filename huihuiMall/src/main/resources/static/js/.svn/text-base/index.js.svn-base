...
(function($) {
	$.learuntab = {
		...
		addTab: function() {
			...
			if(flag) {
				var str = '<a href="javascript:void(0);" class="active menuTab" data-id="' + dataUrl + '">' + menuName + ' <i class="fa fa-remove"></i></a>';
		...
		loadMenu: function() {
            ...
            $.ajax({
                ...
                success: function (retInfo) {
                	console.log(retInfo);
                	var loginAccountType = retInfo.data.accountType;
                    console.log(loginAccountType);
                    var retData = retInfo.data.model;
					/*生成左侧导航栏菜单*/
                    if(retData.children != null) {
                        ...
                        for(var i = 0; i < parentList.length; i++) {
							...
							_html += '<i class="fa fa-gear fa-fw"></i><span>' + parentList[i].name + '</span><i class="fa fa-angle-left pull-right"></i>';
							_html += '</a>';
                            if(parentList[i].children != null) {
                                ...
                                for(var k = 0; k < childrenList.length; k++) {
                                    _html += '<li>';
                                    var childThird = childrenList[k].children;
                                    if(childThird != null) {
                                        _html += '<a href="javascript:void(0)"><i class="fa fa-gear fa-fw"></i>' + childrenList[k].name + '';
                                        _html += '<i class="fa fa-angle-left pull-right"></i></a>';
                                        _html += '<ul class="treeview-menu">';
                                        for(var m = 0; m < childThird.length; m++) {

                                        	var str = JSON.stringify(childThird[m].resourceUrl);
                                            var url = str.substring(1,str.length-4);

                                            _html += '<li>';
                                            _html += '<a class="menuItem" id="' + childThird[m].id + '" data-id="' + childThird[m].id + '" href="'+server_context+''+url+'"><i class="fa fa-gear fa-fw"></i>' + childThird[m].name + '</a>';
                                            _html += '</li>'
                                        }
                                        _html += '</ul>';
                                    } else {
                                        ...
                                        _html += '<a class="menuItem" id="' + childrenList[k].id + '" data-id="' + childrenList[k].id + '" href="'+server_context+''+url+'"><i class="fa fa-gear fa-fw"></i>' + childrenList[k].name + '</a>';
                                    }
                                    _html += '</li>';
                                }
                                _html += '</ul>';
                        ...
                        if(loginAccountType == 3){
                            $("#16").css("display","none"); //资质审核信息查询
                            $("#30").css("display","none"); //入库管理
                            $("#32").css("display","none"); //订单出库管理
                            $("#53").css("display","none"); //换货出库管理
                            $("#34").css("display","none"); //调库管理
                            $("#52").css("display","none"); //调库审批
                            $("#40").css("display","none"); //自营退换货
						}
