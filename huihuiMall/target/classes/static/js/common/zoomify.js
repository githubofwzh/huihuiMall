...
;(function($){
	...
	Zoomify.prototype.zoomIn = function () {
		...
		var offset     = this.$image.offset(),
			...
			wWidth     = $(window).width(),
			wHeight    = screen.availHeight,//$(window).height(),
			...
			translateY = (-offset.top-100 +(wHeight - height)/ 2 + $(window.parent).scrollTop()) / scale;
		if($(window.parent).scrollTop()<100){
            translateY = (-offset.top-60 +(wHeight - height)/ 2 + $(window.parent).scrollTop()) / scale;
		}else {
            translateY = (-offset.top-100 +(wHeight - height)/ 2 + $(window.parent).scrollTop()) / scale;
		}
