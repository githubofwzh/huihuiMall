(function(factory) {
    ...
    var RULES = {
        ...
        isPrizePro: function(value, errorMsg) {
            //中奖概率
            if (!/^(\d{1,2}(\.\d{1,3})?|100)$/.test(value)) {
                return errorMsg;
            }
        },
        isRoleEnable: function(value, errorMsg) {
            //选择是否启用
            if (value==-1) {
                return errorMsg;
            }
        },
        isRole: function(value, errorMsg) {
            //是否是请选择选项
            if (value==0) {
                return errorMsg;
            }
        },
        isSelectNull: function(value, errorMsg) {
            //是否是请选择选项
            if (value=="") {
                return errorMsg;
            }
        ...
        isRegisterSame: function(value, errorMsg) {
            //注册密码是否相同
            if ($("#passWord").val()!=value) {
                return errorMsg;
            }
        },
        isSamll: function(value, errorMsg) {
            //添加-下限是否小于上限
            if ($("#prizeStrategyTop").val()!="") {
                if(value>parseInt($("#prizeStrategyTop").val())){
                    return errorMsg;
                }
            }
        },
        isBig: function(value, errorMsg) {
            //添加-上限是否大于下限
            if ($("#prizeStrategyBottom").val()!="") {
                if(parseInt($("#prizeStrategyBottom").val())>value){
                    return errorMsg;
                }
            }
        },
        ismodifySamll: function(value, errorMsg) {
            //修改-下限是否小于上限
            if ($("#modifyPrizeStrategyTop").val()!="") {
                if(value>parseInt($("#modifyPrizeStrategyTop").val())){
                    return errorMsg;
                }
            }
        },
        ismodifyBig: function(value, errorMsg) {
            //修改-上限是否大于下限
            if ($("#modifyPrizeStrategyBottom").val()!="") {
                if(parseInt($("#modifyPrizeStrategyBottom").val())>value){
                    return errorMsg;
                }
            }
        },
        isSame: function(value, errorMsg) {
            //重置密码是否相同
        ...
        largeOne: function(value, errorMsg) {
            //额度大于1
            if (value < 1) {
                return errorMsg;
            }
        ...
        onlyIntNum: function(value, errorMsg) {
            //积分数值是否为0
            if (value!=0){
                if (!/^[0-9]*$/.test(value)) {
                    return errorMsg;
    ...
    var plugin = {
        ...
        submitValidate: function(options) {
            ...
            if (result.errorMsg) {
                $target = $(result.el);
                //隐藏注册的提示信息
                $target.next('span[class="tipInfo"]').css("display","none");
