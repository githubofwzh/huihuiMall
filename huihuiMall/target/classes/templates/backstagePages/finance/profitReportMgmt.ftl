...
<script>
 ...
 var header = $("meta[name='_csrf_header']").attr("content");
    ...
    //获取信息方法
    function getUserInfos(pageCount,currentPageNum,startingDate,endingDate){
         var startingDate = $("#startingDate").val();
         var endingDate = $("#endingDate").val();
        toastr.options = {
        ...
        $.ajax({
            type : "POST", //用POST方式传输
            url : '${bath}/financialReport/shareProfit', //目标地址
            data : {
            ...
            success : function(retInfo) {
                ...
                if(retCode != 0){
                    ...
                    for(var i = 0 ;i < retDataLength; i ++){
                        ...
                        var shelves_date = retData[i].shelves_date;
                        var tbodyHtml = "<tr>";
                        tbodyHtml += "<td>"+index+"</td>";
                        tbodyHtml += "<td>"+shelves_date+"</td>";
   ...
   function excelExport(pageCount,currentPageNum,startingDate,endingDate){
      ...
      var param={
          ...
          window.location.href="${bath}/FinanncialReportExecel/shareProfit?startTime="+startingDate+"&endTime="+endingDate;
        }
    //重置实现方法
    function reset(){
