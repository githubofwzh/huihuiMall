...
<script>
    ...
    $(document).ready(function() {
      ...
      $("#viewPrint").click(function(){
          var datetime = $("#month").val();
          viewPrint(pageCount,1,datetime);
            });
        //重置点击
    ...
    function getUserInfos(pageCount,currentPageNum){
        ...
        $.ajax({
            type : "POST", //用POST方式传输
            url : '${bath}/financialReport/bestSell', //目标地址
            data : {
       ...
       function excelExport(pageCount,currentPageNum,datetime){
           ...
           var  param = {
                          date : datetime };
            window.location.href="${bath}/FinanncialReportExecel/bestSell?param="+param;
            }
...
//浏览打印
    function viewPrint(pageCount,currentPageNum,datetime){
