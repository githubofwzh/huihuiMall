...
<label id="recordId" style="display:none">${id}</label>
    ...
    <div class="row">
        <div class="col-lg-6 col-md-6">
            ...
            <#--<div class="col-lg-12 col-md-12 input-form">
                ...
                <div class="col-lg-12 col-md-12 input-form">
                    <div class="col-lg-12 col-md-12 _padding" id="materialsPanel">
                        <div class="col-lg-3 col-md-3 title">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $.ajax({
            ...
            success : function(retInfo) {
                ...
                console.log(retInfo);
                selectedMedicineId = retData.medicineid;
                /* selectedSourceId = retData.sourceid;*/
                ...
                $("#specification").val(medicineSpecification);
                $("#salesWeight").val(salesWeight);
                $("#applyCount").val(applyCount);
                $("#totalPrice").val(totalPrice);

                $("#medicineId").text(retData.medicineid);
                $("#typeCheckboxSelected").text("d_"+retData.medicineid);
                /*$("#sourceId").text(retData.sourceid);
                $("#sourceCheckboxSelected").text("s_"+retData.sourceid);*/

                var crudeMedicine = retData.crudeMedicineEntityList;
                var crudeMedicinelength = crudeMedicine.length;

                for(var k = 0; k < crudeMedicinelength; k ++){
                    var id = crudeMedicine[k].id;
                    var name = crudeMedicine[k].name;
                    var aidbasename = crudeMedicine[k].aidbasename;

                    var html ="<tr class='gradeA odd' role='row' id='"+id+"'>";
                    html += "<td>"+name+"</td>";
                    html += "<td>"+aidbasename+"</td>";
                    html += "<td><a href='javascript:void(0);' onclick='removeTr("+id+");'>移除</a></td>";
                    html += "</tr>";
        ...
        $("#submitBtn").click(function(){
            ...
            if(batchNumParam == "" || medicineIdParam == "" || medicineSpecificationParam == "" || salesWeightParam  == ""|| applyCountParam == "" || totalPriceParam == ""){
                ...
                $.ajax({
                    type : "POST", //用POST方式传输
                    url : '${bath}/apply/updateApply', //目标地址
                    data : {
                        id : recordId,
                ...
                if(retData == null){
                    ...
                    for(var k = 0 ; k < retDataLength ; k ++){
                        ...
                        if(selectedMedicineId == medicineId){
                            trMedicineHtml += "<td><input type='checkbox' checked='checked'  class='checkbox checkbox2' id='d_"+medicineId+"' style='position: absolute;margin-top:4px'/></td>";
                        }else{
                ...
                if(retData == null){
                    ...
                    for(var i = 0 ; i < retDataLength ; i ++){
                        ...
                        /*if(selectedSourceId == sourceId){
                            trSourceHtml += "<td><input type='checkbox' checked='checked' class='checkbox checkbox2' id='s_"+sourceId+"' style='position: absolute;margin-top:4px'/></td>";
                        }else{
                        }*/
    ...
    $("#chooseDrugsBtn").click(function(){
        getMedicinetype(pageCount,currentPageNum,search_key);
    });

    $("#chooseSourceBtn").click(function(){
        getSource(pageCount,currentPageNum,search_key2);
    });
    ...
    /*function confirmSource(){
        ...
        var typeStr = $("#sourceCheckboxSelected").text();
        if(typeStr == ""){
            toastr.error('请选择一个产地来源！');
            return false;
        }else{
            var typeNameStr = typeStr+"_name";
            var typeName = $("#"+typeNameStr).html();
            $("#sourceName").val(typeName);

            var typeId = typeStr.split('_')[1];
            $("#sourceId").html(typeId);
        }
        $('#chooseSourceModal').modal('hide')
    };*/
</script>